...
static milky_bool_t request_and_wait(const milky_console_host_api_t *host_api) {
    char response[128];
...
static milky_bool_t MILKY_CONSOLE_CALL on_load(const char *config_json, const milky_console_host_api_t *host_api) {
    (void)config_json;
    return host_api != NULL && request_and_wait(host_api);
}

static milky_result_t MILKY_CONSOLE_CALL on_unload(void) { return MILKY_RESULT_OK; }
static void MILKY_CONSOLE_CALL on_message(const char *message) { (void)message; }
